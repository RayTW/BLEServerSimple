...
public class ANCSGattCallback extends BluetoothGattCallback {
	...
	public void onCharacteristicChanged(BluetoothGatt gatt,
		...
		EZLog.dGlobal("測試::onCharacteristicChanged,uuid["+uuid+"]");
	...
	public void onConnectionStateChange(BluetoothGatt gatt, int status,
			int newState) {
		EZLog.dGlobal("測試::onConnectionStateChange,status["+status+"],newState["+newState+"]");
	...
	public void onServicesDiscovered(BluetoothGatt gatt, int status) {
		EZLog.dGlobal("測試::onServicesDiscovered,status["+status+"]");
		...
		ToolUtils.makeTextToast(mContext, "find ANCS service");
	...
	public void onDescriptorWrite(BluetoothGatt gatt,
		...
		EZLog.dGlobal("測試::onDescriptorWrite,status["+status+"]");
		...
		if(mContext != null){
			if(status == 5 || status == 133){
				ToolUtils.makeTextToast(mContext, "status = " + status);
			}
		}
...
//		.setSmallIcon(R.drawable.ic_launcher)
		...
		//
		BLEManager.getInstance(mContext).setBLELog("ancs,title[" + mnoti.title + "],msg[" + mnoti.message + "]");
	}
